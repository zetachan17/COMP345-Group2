...
CommandProcessor *GameEngine::initializeCommandProcessor()
{
    ...
    if (userInput.substr(0,2) == "-M")
    {
        ...
        commandProcessor2->commands = commandProcessor2->processTournamentCommand(userInput);
        for (std::string i : commandProcessor2->commands)
        {
            std::cout << i << std::endl;
        }
...
//std::string GameEngine::processTournamentCommand(string userinput)
//{
...
//    tournamentFile.open("tournamentFile.txt", ios::trunc);
